...
const insertLoginHistory = async ({
  ...
  await pool.query(
    `
      ...
      VALUES ($1, $2, $3, $4, $5, $6);
    `,
    [userId, loginStatus, ipAddress, userAgent, attemptedIdentifier, failureReason],
  );
};
...
const buildAuthResponse = (user) => {
  const sanitizedUser = mapUserProfile(user);
  ...
  try {
    ...
    if (!user) {
      await insertLoginHistory({
        ...
        ipAddress,
        userAgent,
        attemptedIdentifier: loginIdentifier,
    ...
    if (!passwordMatches) {
      const nextFailedAttempts = Number(user.failed_login_attempts) + 1;
      const shouldLock = nextFailedAttempts >= getMaxFailedLoginAttempts();
      ...
      await pool.query(
        `
          ...
          WHERE id = $1;
        `,
        [user.id, nextFailedAttempts, lockedUntil],
      );

      await insertLoginHistory({
        userId: user.id,
        loginStatus: shouldLock ? "LOCKED_OUT" : "FAILED",
        ipAddress,
        userAgent,
        attemptedIdentifier: loginIdentifier,
        failureReason: shouldLock ? "MAX_ATTEMPTS_REACHED" : "INVALID_PASSWORD",
      });
    ...
    const refreshedUser = await findUserByIdentifier(loginIdentifier);
    res.json({
      success: true,
...
router.post("/logout", authenticate, async (req, res) => {
  try {
    ...
    await insertLoginHistory({
      ...
      ipAddress,
      userAgent,
      attemptedIdentifier: req.user.username,
    });
...
router.post("/change-password", authenticate, async (req, res) => {
  try {
    ...
    const passwordMatches = await verifyPassword(currentPassword, existingUser.password_hash);
    if (!passwordMatches) {
    ...
    await pool.query(
      `
        ...
        WHERE id = $1;
      `,
      [req.user.id, passwordHash, passwordExpiresAt],
    );
