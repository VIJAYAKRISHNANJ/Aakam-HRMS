...
export const getMaxFailedLoginAttempts = () => {
  const parsed = Number(process.env.AUTH_MAX_FAILED_LOGIN_ATTEMPTS);
  return Number.isInteger(parsed) && parsed >= 3 && parsed <= 10
    ? parsed
    : DEFAULT_MAX_FAILED_LOGIN_ATTEMPTS;
};

export const getLockoutMinutes = () => {
  const parsed = Number(process.env.AUTH_LOCKOUT_MINUTES);
  return Number.isInteger(parsed) && parsed >= 5 && parsed <= 60
    ? parsed
    : DEFAULT_LOCKOUT_MINUTES;
};

export const getPasswordExpiryDays = () => {
  const parsed = Number(process.env.PASSWORD_EXPIRY_DAYS);
...
export const hashPassword = async (password) => {
  ...
  return bcrypt.hash(normalized, getBcryptSaltRounds());
};
...
export const isAccountLocked = (lockedUntil) => {
...
export const getPasswordExpiryDate = () => {
  ...
  expiresAt.setDate(expiresAt.getDate() + getPasswordExpiryDays());
  return expiresAt;
};
