...
function Workforce() {
  ...
  */
  ...
  const canUpdateEmployees = hasPermission("workforce.update");

  const canDeleteEmployees = hasPermission("workforce.delete");
  ...
  */
  ...
  }, [canViewEmployees, search, departmentId, status]);
  ...
  */
  ...
  const handleDeleteEmployee = async () => {
    if (!canDeleteEmployees || !employeeToDelete || deleting) {
      return;
    }
    ...
    try {
      ...
      if (statusCode === 404) {
        setDeleteError("This employee no longer exists in the database.");

        return;
      }

      setDeleteError(
        backendMessage || "Unable to delete this employee. Please try again.",
      );
    } finally {
      setDeleting(false);
    }
  ...
  */
  ...
  const handleExport = () => {
    ...
    link.download = "aakam-employees.csv";
          ...
          <section className="rounded-xl border border-slate-200 bg-white p-8 shadow-sm">
            <div className="mx-auto max-w-xl text-center">
              ...
              <p className="mt-2 text-sm leading-6 text-slate-500">
                You do not have permission to view the company employee
                directory.
              </p>
        ...
        <section className="flex flex-col items-start justify-between gap-4 sm:flex-row sm:items-center">
          <div>
            ...
            <p className="mt-1 text-sm text-slate-600">
              {loading
                ? "Loading employees..."
                : `${totalEmployees.toLocaleString("en-IN")} Total Employees`}
            </p>

            {!loading && totalEmployees > 0 && (
              <p className="mt-1 text-xs text-slate-400">
                {activeEmployees} active employees in the current view
              </p>
            )}
          </div>
                  ...
                  employees.map((employee) => (
                    ...
                    >
                      ...
                      <td className="max-w-0 overflow-hidden px-3 py-3">
                        ...
                        >
                          {formatEmploymentType(employee.status)}
                        </span>
                      </td>
            ...
            <div className="w-full max-w-md rounded-2xl border border-slate-200 bg-white p-6 shadow-2xl">
              ...
              <div className="mb-6 rounded-lg border border-amber-200 bg-amber-50 px-4 py-3">
                <p className="text-xs leading-5 text-amber-800">
                  This permanently removes the employee from the database if
                  there are no related HR records. Employees with related
                  records must be deactivated instead.
                </p>
              </div>
