...
function ExitDetails() {
  ...
  const canViewAnyExit =
    canViewExits || canViewOwnExits;

  const [record, setRecord] =
    useState<ExitRecord | null>(null);

  const [checklist, setChecklist] =
    useState<ExitChecklistItem[]>([]);

  const [settlement, setSettlement] =
    useState<ExitSettlement | null>(null);

  const [documents, setDocuments] =
    useState<ExitDocument[]>([]);
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      /*
       * Load the parent exit record first. Backend authorization is
       * authoritative for tenant/self scope. Nested resources are only
       * requested after the parent record is successfully authorized.
       */
      const exit = await getExit(id);
  ...
  const run = async (
    ...
    if (!canUpdateExit) {
      setError(
        "You do not have permission to update exit information.",
      );
      return;
    }
    ...
    try {
      ...
      setError(
        getExitErrorMessage(
          requestError,
          "Unable to update exit information.",
        ),
      );
    } finally {
      setBusy("");
  ...
  const moveWorkflow = (next: ExitStatus) => {
    if (!id || !record || !canUpdateExit) return;
  ...
  const saveSettlement = async () => {
    ...
    if (
      settlementDraft.netSettlement >
      settlementDraft.payableAmount
    ) {
      setError(
        "Net settlement cannot exceed payable amount.",
      );
      return;
    }
    ...
    await run(
      ...
      () =>
        updateExitSettlement(id, {
          status:
            settlementDraft.status as SettlementStatus,
          settlementDate:
            settlementDraft.settlementDate,
          payableAmount:
            settlementDraft.payableAmount,
          deductions:
            settlementDraft.deductions,
          netSettlement:
            settlementDraft.netSettlement,
          remarks:
            settlementDraft.remarks,
        }),
    );
        ...
        <div className="flex min-h-[520px] items-center justify-center px-6">
          <div className="max-w-md text-center">
            ...
            <p className="mt-2 text-sm leading-6 text-slate-500">
              You do not have permission to view exit details.
            </p>
