...
function Clients() {
  ...
  const filteredClients = useMemo(
    () =>
      clients.filter((client) => {
        const haystack =
          `${client.clientCode} ${client.clientName} ${client.contactPerson ?? ""} ${client.email ?? ""}`.toLowerCase();
        return (
  ...
  const removeClient = async () => {
    ...
    try {
      ...
      setSuccessMessage(
        `${clientToDelete.clientName} was deleted successfully.`,
      );
      setClientToDelete(null);
    } catch {
      setActionError("Unable to delete client. Please try again.");
    } finally {
      setDeletingId(null);
    }
  };
    ...
    <DashboardLayout>
      <div className="flex w-full min-w-0 flex-col gap-6">
        ...
        <section className="rounded-xl border border-slate-300 bg-white p-4">
          <div className="grid grid-cols-1 gap-3 md:grid-cols-[minmax(0,1fr)_180px]">
            ...
            <select
              ...
              className="h-10 rounded-lg border border-slate-300 bg-white px-3 text-sm text-slate-800 outline-none focus:border-teal-600"
            >
        ...
        {loading ? (
          ...
          <section className="rounded-xl border border-slate-300 bg-white px-6 py-16 text-center">
            <Building2 size={32} className="mx-auto mb-3 text-slate-400" />
                  ...
                  {filteredClients.map((client) => (
                    <tr key={client.id} className="hover:bg-slate-50">
                      ...
                      <td className="px-5 py-4">
                        <span
                          className={`inline-flex rounded-full px-2.5 py-1 text-xs font-semibold ${statusClass(client.status)}`}
                        >
                          {client.status}
                        </span>
                      </td>
          ...
          >
            ...
            <div className="mt-7 flex justify-end gap-3">
              ...
              <button
                ...
                disabled={deletingId !== null}
                className="inline-flex min-w-[100px] items-center justify-center rounded-lg bg-rose-600 px-4 py-2.5 text-sm font-semibold text-white hover:bg-rose-700 disabled:opacity-60"
              >
